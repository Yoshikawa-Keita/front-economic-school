...
const ExamYearBlock: React.FC<ExamYearBlockProps> = ({ year, exams, completedExams, fetchCompletedExams}) => {
  ...
  const renderLinkForAll =  (url: string, text: string) => {
    if (!url.includes('undefined')) {
      ...
      if (checkUserType(1) || checkUserType(2)) {
        ...
        // const response = await getSignedUrl(context, {file_path: url})
        return (
            <a href="#" onClick={(e) => handleLinkClick(url, e)} className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">{text}</a>
          )
      }
      return (
        <span className="text-blue-600 opacity-50 cursor-not-allowed">{text}</span>
      )
    } else {
      return (
  ...
  const renderLinkForOne =  (url: string, text: string) => {
    if (checkUserType(2)) {
      if (!url.includes('undefined')) {
        return (
          <a href={url} className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">{text}</a>
        )
      } else {
        return (
          <span className="text-blue-600 opacity-50 cursor-not-allowed">{`${text} (Coming Soon...)`}</span>
      ...
      return (
        <div>
          <span className="text-blue-600 opacity-50 cursor-not-allowed">{text}</span>
          <span className="inline-block bg-red-500 text-white text-xs px-2 py-1 rounded-full ml-2">会員限定</span>
        </div>
      )
    }
  }
        ...
        {exams.map((exam, i) => (
          <div key={i} className="border border-gray-200 p-2 rounded-md">
            ...
            <div className="flex justify-between">
              {renderLinkForAll(`${process.env.NEXT_PUBLIC_CLOUDFRONT_HOST}/${exam.university}/${exam.subject}/question/${exam.question_pdf_url}`, '問題')}
