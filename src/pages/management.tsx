...
const management = () => {
  ...
  const [examCountByUnivercity, setExamCountByUnivercity] = useState<ExamCountByUniversity[]>([])
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
    {examCountByUnivercity.map((examCntByUniv) => {
      ...
      completedExams.map((examUser)  => {
        completed = completedExams.filter(examUser => examUser.university === examCntByUniv.university).length;
        notCompleted = examCntByUniv.count - completed;
      })
      
      return (
        <div key={examCntByUniv.university} className="border border-gray-300 rounded-lg overflow-hidden min-h-[500px] bg-blue-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 h-full">
            <div className="p-4 flex justify-center items-center">
              <div className="w-full h-full">
                <ExamCompletionChart size="w-full h-full" completed={completed} notCompleted={notCompleted} university={examCntByUniv.university} />
              </div>
            </div>
            <div className="p-4 text-center md:self-center">
              <p className="text-lg mb-4"><strong>達成率：</strong>{((completed / (completed + notCompleted)) * 100).toFixed(2)}%</p>
              <p className="text-lg mb-4"><strong>完了数：</strong>{completed}</p>
              <p className="text-lg mb-4"><strong>総数：</strong>{completed + notCompleted}</p>
            </div>
          </div>
        </div>
      );
